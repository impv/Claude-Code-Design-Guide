...
Claude Code には CLAUDE.md、Rules、settings.json、Hooks、Skills、Subagents、MCP サーバーといった機能がある。どれも Claude の動作を変えるものだが、性質が異なる。このガイドでは3つの設計軸——**確定性**、**コンテキストコスト**、**強制度**——で主要機能を位置づけ、「いつ何を使うか」の設計判断の基準を示す。
...
各機能がコンテキストウィンドウ（既定 200K トークン）をどれだけ消費するか。この違いが使い分けの重要な判断基準になる。

| 機能 | コンテキストへの影響 |
|---|---|
| settings.json / Hooks | **定義自体は消費ゼロ**。ハーネス側で処理される（ただし Hooks の実行出力はコンテキストに入る） |
| CLAUDE.md + paths なし Rules | **常時消費**。セッション中ずっと占有する |
| paths あり Rules | **条件付き**。マッチするファイルを読んだ時だけロード |
| Skills の description | **常時消費（微量）**。1,536文字を超える分は切り捨て |
...
| 中 | settings.json の `deny: ["Edit(.env)"]` | Edit ツール経由は確実にブロック | Bash 経由はコマンドのパターン認識どまりで回避されうる |
...
**deny の限界とサンドボックス**: `deny: ["Read(.env)"]` は Read ツールを確実にブロックし、Bash 経由の `cat` や `head` 等の既知コマンドにも適用される。ただしこれはコマンド文字列のパターン認識なので、想定外のコマンドや間接的な読み方までは防げない。確実に遮断したいなら、OS レベルで働くサンドボックスを使う。サンドボックスにも抜け道（Bash ツールのパラメータ `dangerouslyDisableSandbox`）があるため、完全に閉じるには `allowUnsandboxedCommands: false` を併せて設定する。

**設定スコープと優先順位**: Claude Code の設定は managed > CLI 引数 > local > project > user の優先順位に従う。managed（組織）が最優先で下位はオーバーライドできない。配列フィールドは全スコープから結合、非配列フィールドは高優先度が勝つ。なお CLI 引数（`--model`, `--permission-mode` 等）は managed 以外のファイル設定より優先されるが、永続的な設定ではなく実行時の一時的なオーバーライド。
...
| `.env` を Bash 経由でも読ませない | サンドボックス denyRead | deny ルールはコマンドのパターン認識どまりで、あらゆる読み方までは防げない |
| コード修正の反復で毎回の確認を減らしたい | acceptEdits モード | allow に個別追加するより、モード切替の方が手軽 |
| CI/CD パイプラインで無人実行したい | dontAsk モード | 権限確認で CI が止まる。allow ルール外は自動拒否になる |
...
**コンテキスト管理上の注意**: 一度起動された Skill の本文（前処理の結果を含む）はコンテキストに残り続ける。コンテキスト圧縮が起きると各スキルは要約の後ろに再添付されるが、その枠は「各スキル先頭 5,000 トークン・合計 25,000 トークン」で、新しく起動したものが優先される。つまり1セッションで多くのスキルを起動すると古いものから消える。スキルは軽く保ち、必要なものだけ起動するのが原則。
...
**Skills との根本的な違い**: コンテキストの隔離。Skills はメインのコンテキスト内で動き会話履歴が見えるが、Subagents は別コンテキストで動き親の会話履歴を知らない（CLAUDE.md や Git ステータス等の初期コンテキストはロードされる）。メインのコンテキストを消費しないのが最大の利点。
