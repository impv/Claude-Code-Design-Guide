...
| Skills の本文 | **起動時に消費**。一度読み込まれるとセッション中残る |
...
**コンテキスト管理上の注意**: 一度起動された Skill の本文（前処理の結果を含む）はセッション中ずっと会話に残る。大きな参考資料を Skill に入れる場合、起動後のコンテキスト消費を意識する。頻繁に起動する軽いスキルと、大きな参考資料を含む重いスキルは分けた方がよい。
...
| 軽い調査をコスト抑えて実行したい | Subagents（model: haiku） | Skills はメインのモデルで動く |
